...
int main(void)
{
    EGLDisplay display=EGL_NO_DISPLAY;EGLContext context=EGL_NO_CONTEXT;EGLSurface surface=EGL_NO_SURFACE;kr3d_gl_device*device=NULL;kr3d_mesh_handle mesh=0;kr3d_material_handle material=0;int result=EXIT_FAILURE;
    ...
    kr3d_error error={0};kr3d_device_desc dd={sizeof dd,4,4,4,64,64};kr3d_gl_options options={sizeof options,true,true};CHECK(kr3d_gl_device_create(&dd,&options,&device,&error));CHECK(kr3d_gl_renderer(device)!=NULL);
