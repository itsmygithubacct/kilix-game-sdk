...
int main(void)
{
    ...
    CHECK(kr3d_gl_device_create(&dd,&options,&device,&error));CHECK(kr3d_gl_renderer(device)!=NULL);uint32_t texel=UINT32_C(0xff00ff00);kr3d_texture_desc td={.struct_size=sizeof td,.width=1,.height=1,.rgba=&texel,.filter=KR3D_FILTER_NEAREST,.address_u=KR3D_ADDRESS_CLAMP,.address_v=KR3D_ADDRESS_CLAMP};CHECK(kr3d_gl_texture_create(device,&td,&texture,&error));texel=UINT32_C(0xff0000ff);kr3d_texture_update_desc tu={.struct_size=sizeof tu,.width=1,.height=1,.rgba=&texel};CHECK(kr3d_gl_texture_update(device,texture,&tu,&error));tu.x=1;CHECK(!kr3d_gl_texture_update(device,texture,&tu,&error)&&error.code==KR3D_ERROR_ARGUMENT);tu.x=0;
